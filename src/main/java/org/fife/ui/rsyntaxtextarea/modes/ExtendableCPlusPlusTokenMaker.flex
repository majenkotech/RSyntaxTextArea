...
%public
%class ExtendableCPlusPlusTokenMaker
...
BinaryLiteral		= ("0b"{BinaryDigit}+{NonFloatSuffix}?)
HexLiteral		= ("0"[xX]{HexDigit}+{NonFloatSuffix}?)
FloatLiteral		= ((({Digit}*[\.]{Digit}+)|({Digit}+[\.]{Digit}*)){Exponent}?[fFlL]?)
ErrorNumberFormat	= (({BinaryLiteral}|{IntegerLiteral}|{HexLiteral}|{FloatLiteral}){NonSeparator}+)
...
<YYINITIAL> {
    ...
    {DefOutBegin}               { start = zzMarkedPos-5; yybegin(DEFOUT); }
...
<DEFOUT> {
	...
	[^hwf\n\*]+			{}
	{URL}				{ int temp=zzStartRead; addToken(start,zzStartRead-1, Token.COMMENT_MARKUP); addHyperlinkToken(temp,zzMarkedPos-1, Token.COMMENT_MARKUP); start = zzMarkedPos; }
	[hwf]				{}

	\n					{ addToken(start,zzStartRead-1, Token.COMMENT_MARKUP); return firstToken; }
	{DefOutEnd}			{ yybegin(YYINITIAL); addToken(start,zzStartRead+5, Token.COMMENT_MARKUP); }
	\*					{}
	<<EOF>>				{ addToken(start,zzStartRead-1, Token.COMMENT_MARKUP); return firstToken; }
